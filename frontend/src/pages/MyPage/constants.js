...
export const ROLE_LABEL = { USER: "시민 사용자", STAFF: "담당 직원", ADMIN: "관리자" };
...
export const SAFETY_STATUS_LABEL = { PENDING: "응답 대기중", SAFE: "안전 확인 완료", HELP: "도움 필요" };
export const SAFETY_STATUS_STYLE = {
  PENDING: "bg-amber-100 text-amber-700",
  SAFE: "bg-emerald-100 text-emerald-700",
  HELP: "bg-red-100 text-red-700",
};
export const SAFETY_STATUS_ICON = { PENDING: Clock, SAFE: CheckCircle2, HELP: AlertTriangle };
...
export const buildNotificationItems = ({ sentChecks, receivedChecks, myReports, regions, dbNotifications = [] }) => {
  const items = [];
  ...
  // 시점에 만들어둔 진짜 알림. type은 백엔드 "DISASTER"/"REPORT"를 소문자로 맞춰줌.
  dbNotifications.forEach((n) => {
    items.push({
      id: `db-${n.notificationId}`,
      type: n.type === "DISASTER" ? "disaster" : "report",
      title: n.title,
      desc: n.content,
...
export const SEVERITY_LABEL_KO = { HIGH: "위험도 높음", MEDIUM: "위험도 보통", LOW: "위험도 낮음" };
...
// 행안부 긴급재난문자 원본 데이터(RCPTN_RGN_NM)는 발송 기관마다 시/도를 정식명("대전광역시")으로 쓰기도 하고
// 약칭("부산")으로 쓰기도 해서 표기가 들쭉날쭉함 - 정식명으로 조회했다가 0건이면 약칭으로 한 번 더 재시도하기 위한 매핑
