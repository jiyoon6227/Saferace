...
BEGIN
  -- 위에서 만든 회원 2명의 ID 조회
  SELECT MEMBER_ID INTO v_citizen_id FROM SF_MEMBER WHERE LOGIN_ID = 'u';
  SELECT MEMBER_ID INTO v_staff_id FROM SF_MEMBER WHERE LOGIN_ID = 's';
  ...
  -- [관심지역 테스트 데이터] 각 계정에 관심지역 1~2개씩 등록 (좌표/라벨 포함)
  INSERT INTO SF_MEMBER_REGION (MEMBER_REGION_ID, MEMBER_ID, REGION_NAME, LATITUDE, LONGITUDE, REGION_LABEL, IS_PRIMARY)
  VALUES (SEQ_SF_MEMBER_REGION.NEXTVAL, v_citizen_id, '대전광역시 유성구 어은동', 36.3625, 127.3453, '우리집', 'Y');

  INSERT INTO SF_MEMBER_REGION (MEMBER_REGION_ID, MEMBER_ID, REGION_NAME, LATITUDE, LONGITUDE, REGION_LABEL, IS_PRIMARY)
  VALUES (SEQ_SF_MEMBER_REGION.NEXTVAL, v_citizen_id, '세종특별자치시', 36.4800, 127.2890, '관심지역', 'N');

  INSERT INTO SF_MEMBER_REGION (MEMBER_REGION_ID, MEMBER_ID, REGION_NAME, LATITUDE, LONGITUDE, REGION_LABEL, IS_PRIMARY)
  VALUES (SEQ_SF_MEMBER_REGION.NEXTVAL, v_staff_id, '대전광역시 서구', 36.3290, 127.3825, '우리집', 'Y');
